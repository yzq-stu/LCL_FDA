...
from LCL_FDA import Network
from tester import ModelTester
from helper_ply import read_ply
from helper_tool import ConfigPowerLine_1 as cfg
...
class PowerLine:
    def __init__(self, test_area_idx):
        ...
        self.test_files = glob.glob(join(self.path, 'original_ply', self.val_split+'.ply'))
        # Initiate containers
        self.val_proj = []
        self.val_labels = []
        self.possibility = {}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='continue', help='options: train, test,continue')
    ...
    #         tf.saved_model.loader.load(sess, [tf.saved_model.tag_constants.SERVING],chosen_snap)
    #
    ...
    #         if var.name.startswith('loaded_model'):
    #             var_value = sess.run(var)
    #             var_name = var.name.replace('loaded_model', 'new_model')
    #             new_var = tf.get_variable(var_name, shape=var_value.shape, dtype=tf.float32,
    #                                       initializer=tf.constant_initializer(var_value))
    #             var.assign(new_var)
    #     ##3 train
    #     model.train(dataset)
    elif Mode == 'continue':
        ...
        if FLAGS.model_path is not 'None':
            ...
            chosen_step = np.sort(snap_steps)[-1]
            chosen_snap = os.path.join(snap_path, 'snap-{:d}'.format(chosen_step))
        model = Network(dataset, cfg, chosen_snap)
        model.train(dataset)
    else:
        ##################
        # Visualize data #
