...
class ConfigPowerLine_1:
    k_n = 16  # KNN[8,16,32,64]
    ...
    val_batch_size = 14  # batch_size during validation and test  14
    ...
    # d_out = [16, 32, 64, 128, 256]  # feature dimension
    sub_point = [10240, 2560, 640, 160]
    noise_init = 3.5  # noise initial parameter
    max_epoch = 250  # maximum epoch during training
    learning_rate = 1e-2  # initial learning rate
    # lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate of learning rate
    lr_decays = {i: 0.7 if i % 10 == 0 else 1 for i in range(1, 501)}
    lr_decays[0] = 1
...
class ConfigPowerLine_2:
    k_n = 16  # KNN
    num_layers = 5  # Number of layers
    num_points = 40960  # Number of input points
    num_classes = 8  # Number of valid classes
    sub_grid_size = 0.2  # preprocess_parameter 0.4
    ...
    # d_out = [16, 32, 64, 128, 256]  # feature dimension
    sub_point = [10240, 2560, 640, 160, 80]
    noise_init = 3.5  # noise initial parameter
    max_epoch = 250  # maximum epoch during training
    learning_rate = 1e-2  # initial learning rate
    # lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate of learning rate
...
class ConfigToronto3D:
    ...
    val_steps = 100  # Number of validation steps per epoch  ##25  100
    sub_point = [10240, 2560, 640, 160, 80]
    sub_sampling_ratio = [4, 4, 4, 4, 2]  # sampling ratio of random sampling at each layer
    d_out = [16, 64, 128, 256, 512]  # feature dimension

    noise_init = 3.5  # noise initial parameter
    max_epoch = 500  # maximum epoch during training
    learning_rate = 1e-2  # initial learning rate
    # lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate of learning rate

    lr_decays = {i: 0.7 if i % 10 == 0 else 1 for i in range(1, 501)}
    lr_decays[0] = 1
    # lr_decays = {i: 0.99 for i in range(0, 500)}
    train_sum_dir = 'train_log'
    saving = True
    saving_path = None
...
class ConfigS3DIS:
    ...
    sub_grid_size = 0.04  # preprocess_parameter 0.4
...
class DataProcessing:
    ...
    @staticmethod
    def OA_mAcc_from_confusions(confusions):
        TP = np.diagonal(confusions, axis1=-2, axis2=-1)
        FP = np.sum(confusions, axis=0) - TP
        TN = np.sum(confusions) - (TP + FP)
        FN = np.sum(confusions, axis=1) - TP
        OA = (TP + TN) / (TP + FP + FN + TN)
        mAcc = (TP / (TP + FP) + TN / (FN + TN)) / 2
        return OA,mAcc
    ...
    def get_class_weights(dataset_name):
        ...
        if dataset_name is 'S3DIS':
            ...
            num_per_class = np.array([68417, 65488, 12396094, 827564, 452362, 2827, 8334, 4510463],
                                      dtype=np.int32)  ##1833 1549  Toronto3d
        elif dataset_name is 'PowerLine':
            num_per_class = np.array([137579,80135,21874964, 1799151, 358902, 3673,10303, 7019496],
                                     dtype=np.int32)
            """
            num_per_class = np.array([68417, 65488,12396094, 827564, 452362, 2827, 8334, 4510463],
                                     dtype=np.int32)
