...
class ModelTester:
    def __init__(self, model, dataset, restore_snap=None):
        ...
        # Load trained model 加载模型
        if restore_snap is not None:
            self.saver.restore(self.sess, restore_snap)
            print("Model restored from " + restore_snap)

        self.prob_logits = tf.nn.softmax(model.logits)      ##  softmax
    ...
    def test(self, model, dataset, num_votes=100):
        ...
        while last_min < num_votes:
            try:
                ...
                t_pred0_1 = time.time()
                t2.append(t_pred0_1 - t_pred0_0)
                t1 = t1 + (t_pred0_1 - t_pred0_0)
                # print('Step prediction time: {:.3f} s\n'.format(t_pred0_1 - t_pred0_0))
                print('prediction time total: {:.3f} s\n'.format(t1))
                ...
                t_pred2 = time.time()
                t = t + (t_pred2 - t_pred1)
                # print('Step prediction done in {:.1f} s\n'.format(t_pred2 - t_pred1))
                ...
                print('Total prediction time {:.1f} s\n'.format(t))
                ...
                if last_min + 1 < new_min:
                    ...
                    if int(np.ceil(new_min)) % 1 == 0:
                        ...
                        IoUs = DP.IoU_from_confusions(C)
                        OAs, mAccs = DP.OA_mAcc_from_confusions(C)

                        m_IoU = np.mean(IoUs)
                        OA = np.mean(OAs)
                        mAcc = np.mean(mAccs)
                        s = '{:5.2f} | '.format(100 * m_IoU) + '{:5.2f} | '.format(100 * OA) + '{:5.2f} | '.format(100 * mAcc)
                        for IoU in IoUs:
                            s += '{:5.2f} '.format(100 * IoU)
                ...
                step_id = 0
                continue
        print('inference_time:', np.sum(t2))
        return
